...
#define sep ' '
std::array<char, 13> keymap = {'J', '2', '3', '4', '5', '6', '7', '8', '9', 'T', 'Q', 'K', 'A'};
...
int hasJoker(std::unordered_map<char, int> counter) {
  auto it = counter.find('J');
  if(it == counter.end()) return 0; // no joker here
  else return it->second; // returns the amount of jokers
}
...
int getHandType(std::pair<std::string, int> hand) {
  ...
  int jk = hasJoker(counter);
  if(jk > 0 && jk != 5) counter.erase('J');
